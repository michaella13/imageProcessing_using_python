...
while True:
    ...
    if ret:
        ...
        for (x, y, w, h) in faces:
            ...
            eyes = cv2.resize(eyes, None, fx=1.5, fy=1.5, interpolation=cv2.INTER_LINEAR)
            frame[y:y+h, x:x+w] = eyes
            
            # Add a funny hat to the head
            hat = cv2.imread('funny_hat.jpg', cv2.IMREAD_UNCHANGED)
            ...
            frame[y:y+h, x:x+w] = cv2.addWeighted(frame[y:y+h, x:x+w], 1.0, hat_overlay, 0.8, 0)
